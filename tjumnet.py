...
class Teacher_latent(nn.Module):
    def __init__(self, channel_t, channel_h, n_teachers):
        super().__init__()
        self.latent_space = nn.ModuleList()
        for _ in range(n_teachers):
            self.latent_space.append(
                nn.Sequential(
        ...
        self.inverse_latent_space = nn.ModuleList()
        for _ in range(n_teachers):
            self.inverse_latent_space.append(
                nn.Sequential(
    ...
    def forward(self, features):
        assert len(features) == len(self.latent_space)
        projected_features = [self.latent_space[i](features[i]) for i in range(len(features))]
        reconstructed_features = [self.inverse_latent_space[i](projected_features[i]) for i in range(len(projected_features))]

        return projected_features, reconstructed_features


class Student_latent(nn.Module):
    ...
    def forward(self, fs):
        projected_features = self.latent_space(fs)
...
class TFO(nn.Module):
    def __init__(self, channel_t, channel_s, channel_h, n_teachers):
        super().__init__()
        self.teacher_latent = Teacher_latent(channel_t, channel_h, n_teachers)
        self.student_latent = Student_latent(channel_s, channel_h)

    def forward(self, teacher_features, student_feature):
        teacher_projected_feature, teacher_reconstructed_feature = self.teacher_latent(teacher_features)
        student_projected_feature = self.student_latent(student_feature)
